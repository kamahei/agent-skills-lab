...
- For a requested `2-agent` run, do not replace `Current AI model` with an additional non-current reviewer, and do not treat 2 non-current reviewers as satisfying the request.
...
- Fill at most 1 Claude slot per run. Stop the Claude fallback chain as soon as any Claude model returns a usable independent review report.
...
- Fill at most 1 Gemini slot per run. Stop the Gemini fallback chain as soon as any Gemini model returns a usable independent review report.
